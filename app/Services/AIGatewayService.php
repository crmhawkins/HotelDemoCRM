...
class AIGatewayService
{
    ...
    {
        // [DEMO 2026-04-17] Guard modo demo: devolver respuesta mock sin llamar a ningún LLM
        if (\App\Services\DemoModeService::isStubbed('openai')) {
            $userMsg = '';
            ...
            $mockReply = $this->buildDemoReply($userMsg);
            \App\Services\DemoModeService::stubResponse('openai', 'chatCompletion', [
                'preview' => mb_substr($userMsg, 0, 120),
            ]);
            return [
                'content' => $mockReply,
                'model' => 'demo-mock',
    ...
    /**
     * [DEMO 2026-04-17] Genera una respuesta mock determinística según el contenido
     * del prompt. Cubre los casos más comunes del CRM: categorización de gastos,
     * extracción de datos de facturas, check-in DNI, resumen de emails.
     */
    private function buildDemoReply(string $userMsg): string
    {
        $msg = mb_strtolower($userMsg);

        // Categorización de movimientos bancarios / gastos
        if (str_contains($msg, 'categor') && (str_contains($msg, 'gasto') || str_contains($msg, 'movimiento'))) {
            return 'MANTENIMIENTO';
        }
        // Categorización de emails
        if (str_contains($msg, 'email') && str_contains($msg, 'categor')) {
            return 'reserva';
        }
        // OCR / extracción de factura
        if (str_contains($msg, 'factura') || str_contains($msg, 'invoice')) {
            return json_encode([
        ...
        // DNI / pasaporte / check-in
        if (str_contains($msg, 'dni') || str_contains($msg, 'pasaport') || str_contains($msg, 'passport')) {
            return json_encode([
                'nombre' => 'JUAN',
                ...
                'nacionalidad' => 'ESP',
                'sexo' => 'M',
                'fecha_caducidad' => '2030-06-15',
            ], JSON_UNESCAPED_UNICODE);
        }
        // Default: respuesta genérica
        return '[DEMO] Respuesta simulada. El modo demo está activo y no se realizan llamadas a LLMs externos.';
    }
}
