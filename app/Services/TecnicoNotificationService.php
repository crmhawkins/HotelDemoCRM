...
class TecnicoNotificationService
{
    ...
    private static function enviarMensajeTemplate($phone, $templateName, $parameters = [])
    {
        ...
        // [DEMO 2026-04-17] Guard modo demo: no llamar a Meta API
        if (\App\Services\DemoModeService::isStubbed('whatsapp')) {
            return \App\Services\DemoModeService::stubResponse('whatsapp', 'tecnico.template', [
                'phone' => $phone ?? null,
    ...
    private static function enviarMensajeSimple($phone, $texto)
    {
        ...
        // [DEMO 2026-04-17] Guard modo demo: no llamar a Meta API
        if (\App\Services\DemoModeService::isStubbed('whatsapp')) {
            return \App\Services\DemoModeService::stubResponse('whatsapp', 'tecnico.simple', [
                'phone' => $phone,
                'preview' => mb_substr((string)$texto, 0, 100),
            ]);
        }
