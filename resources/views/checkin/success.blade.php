<x-checkin-layout :token="$token">
    ...
    <script>
        ...
        function generateCardCanvas(callback) {
            ...
            ctx.fillText({!! json_encode($codigosAcceso["apartamento_titulo"] ?? "Suite") !!}, w / 2, 150);
            ...
            ctx.fillStyle = '#6b7280';
            ctx.font = '14px system-ui, sans-serif';
            ctx.fillText('{{ __("Puerta de tu suite") }}', w / 2, yPos);
            yPos += 10;
            ...
            generateCardCanvas(function(canvas) {
                canvas.toBlob(function(blob) {
                    ...
                    if (navigator.canShare && navigator.canShare({ files: [file] })) {
                        navigator.share({
                            title: '{{ __("Códigos de acceso") }} - ' + {!! json_encode($codigosAcceso["apartamento_titulo"] ?? "Suite") !!},
                            text: '{{ __("Código suite") }}: {{ $codigosAcceso["codigo_acceso"] }}' + @if(!empty($codigosAcceso['clave_edificio'])) ' | {{ __("Código hotel") }}: {{ $codigosAcceso["clave_edificio"] }}' + @endif '',
                            files: [file]
                        }).catch(function(err) {
                            ...
                            if (err.name !== 'AbortError') {
                                navigator.share({
                                    ...
                                    text: '{{ __("Código suite") }}: {{ $codigosAcceso["codigo_acceso"] }}' + @if(!empty($codigosAcceso['clave_edificio'])) '\n{{ __("Código hotel") }}: {{ $codigosAcceso["clave_edificio"] }}' + @endif ''
                                }).catch(function() {});
                            }
                        });
                    } else {
