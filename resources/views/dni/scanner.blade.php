...
<head>
    ...
    <title>{{ __('dni.scanner.title') }}</title>
    ...
    <style>
        ...
        * {
        ...
        .guide-frame.detected {
            border-color: var(--booking-success);
            box-shadow: 0 0 20px rgba(13, 115, 119, 0.5);
            animation: pulse-green 2s infinite;
        }
        
        @keyframes pulse-green {
            0%, 100% { border-color: var(--booking-success); }
            50% { border-color: #0D9BA0; }
        }
...
<body>
            ...
            <form id="uploadForm" enctype="multipart/form-data" novalidate onsubmit="event.preventDefault(); return false;">
                ...
                <div class="person-section">
                    ...
                    <div class="upload-section">
                        ...
                        <div class="camera-area" id="frontalArea_0">
                            <div class="camera-container-wrapper" id="frontalCameraWrapper_0" style="display: none;">
                                <div class="camera-container">
                                    <video class="camera-video" id="frontalVideo_0" autoplay muted playsinline></video>
                                    <canvas class="camera-canvas" id="frontalCanvas_0"></canvas>
                                    <div class="guide-frame" id="frontalGuideFrame_0">
                                        <div class="guide-message" id="frontalGuideMessage_0">Coloca el DNI dentro del marco</div>
                                    </div>
                                </div>
                                <div class="camera-controls">
                                    <button type="button" class="camera-btn camera-btn-capture" id="frontalCaptureBtn_0" disabled>📸 Capturar</button>
                                    <button type="button" class="camera-btn camera-btn-flip" id="frontalFlipBtn_0">🔄 Cambiar cámara</button>
                                    <button type="button" class="camera-btn camera-btn-retry" id="frontalRetryBtn_0" style="display: none;">🔄 Reintentar</button>
                                </div>
                            </div>
                            <div id="frontalCameraPlaceholder_0" style="text-align: center;">
                                <div class="upload-icon">📷</div>
                                <div class="upload-text">Elige una opción</div>
                                <div class="upload-hint">Puedes hacer una foto o subir una imagen</div>
                                <div style="display: flex; gap: 12px; justify-content: center; margin-top: var(--spacing-md); flex-wrap: wrap;">
                                    <button type="button" class="camera-btn camera-btn-capture" onclick="initCamera(0, 'frontal')">📸 Activar cámara</button>
                                    <button type="button" class="camera-btn camera-btn-capture" style="background: linear-gradient(135deg, #0D7377 0%, #0A5D61 100%);" onclick="document.getElementById('frontalFile_0').click()">📤 Subir archivo</button>
                                </div>
                            </div>
                            <input type="file" id="frontalFile_0" name="frontal[0]" accept="image/jpeg,image/jpg,image/png" style="display: none;">
                    ...
                    <div class="upload-section">
                        ...
                        <div class="camera-area" id="traseraArea_0">
                            <div class="camera-container-wrapper" id="traseraCameraWrapper_0" style="display: none;">
                                <div class="camera-container">
                                    <video class="camera-video" id="traseraVideo_0" autoplay muted playsinline></video>
                                    <canvas class="camera-canvas" id="traseraCanvas_0"></canvas>
                                    <div class="guide-frame" id="traseraGuideFrame_0">
                                        <div class="guide-message" id="traseraGuideMessage_0">Coloca el DNI dentro del marco</div>
                                    </div>
                                </div>
                                <div class="camera-controls">
                                    <button type="button" class="camera-btn camera-btn-capture" id="traseraCaptureBtn_0" disabled>📸 Capturar</button>
                                    <button type="button" class="camera-btn camera-btn-flip" id="traseraFlipBtn_0">🔄 Cambiar cámara</button>
                                    <button type="button" class="camera-btn camera-btn-retry" id="traseraRetryBtn_0" style="display: none;">🔄 Reintentar</button>
                                </div>
                            </div>
                                <div id="traseraCameraPlaceholder_0" style="text-align: center;">
                                    <div class="upload-icon">📷</div>
                                    <div class="upload-text">Elige una opción</div>
                                    <div class="upload-hint">Puedes hacer una foto o subir una imagen</div>
                                    <div style="display: flex; gap: 12px; justify-content: center; margin-top: var(--spacing-md); flex-wrap: wrap;">
                                        <button type="button" class="camera-btn camera-btn-capture" onclick="initCamera(0, 'trasera')">📸 Activar cámara</button>
                                        <button type="button" class="camera-btn camera-btn-capture" style="background: linear-gradient(135deg, #0D7377 0%, #0A5D61 100%);" onclick="document.getElementById('traseraFile_0').click()">📤 Subir archivo</button>
                                    </div>
                                </div>
                            <input type="file" id="traseraFile_0" name="trasera[0]" accept="image/jpeg,image/jpg,image/png" style="display: none;">
                ...
                @for($i = 1; $i < $numeroAdultos; $i++)
                    ...
                    <div class="person-section">
                        ...
                        <div class="upload-section">
                            ...
                            <div class="camera-area" id="frontalArea_{{ $i }}">
                                <div class="camera-container-wrapper" id="frontalCameraWrapper_{{ $i }}" style="display: none;">
                                    <div class="camera-container">
                                        <video class="camera-video" id="frontalVideo_{{ $i }}" autoplay muted playsinline></video>
                                        <canvas class="camera-canvas" id="frontalCanvas_{{ $i }}"></canvas>
                                        <div class="guide-frame" id="frontalGuideFrame_{{ $i }}">
                                            <div class="guide-message" id="frontalGuideMessage_{{ $i }}">Coloca el DNI dentro del marco</div>
                                        </div>
                                    </div>
                                    <div class="camera-controls">
                                        <button type="button" class="camera-btn camera-btn-capture" id="frontalCaptureBtn_{{ $i }}" disabled>📸 Capturar</button>
                                        <button type="button" class="camera-btn camera-btn-flip" id="frontalFlipBtn_{{ $i }}">🔄 Cambiar cámara</button>
                                        <button type="button" class="camera-btn camera-btn-retry" id="frontalRetryBtn_{{ $i }}" style="display: none;">🔄 Reintentar</button>
                                    </div>
                                </div>
                                <div id="frontalCameraPlaceholder_{{ $i }}" style="text-align: center;">
                                    <div class="upload-icon">📷</div>
                                    <div class="upload-text">Elige una opción</div>
                                    <div class="upload-hint">Puedes hacer una foto o subir una imagen</div>
                                    <div style="display: flex; gap: 12px; justify-content: center; margin-top: var(--spacing-md); flex-wrap: wrap;">
                                        <button type="button" class="camera-btn camera-btn-capture" onclick="initCamera({{ $i }}, 'frontal')">📸 Activar cámara</button>
                                        <button type="button" class="camera-btn camera-btn-capture" style="background: linear-gradient(135deg, #0D7377 0%, #0A5D61 100%);" onclick="document.getElementById('frontalFile_{{ $i }}').click()">📤 Subir archivo</button>
                                    </div>
                                </div>
                                <input type="file" id="frontalFile_{{ $i }}" name="frontal[{{ $i }}]" accept="image/jpeg,image/jpg,image/png" style="display: none;">
                        ...
                        <div class="upload-section">
                            ...
                            <div class="camera-area" id="traseraArea_{{ $i }}">
                                <div class="camera-container-wrapper" id="traseraCameraWrapper_{{ $i }}" style="display: none;">
                                    <div class="camera-container">
                                        <video class="camera-video" id="traseraVideo_{{ $i }}" autoplay muted playsinline></video>
                                        <canvas class="camera-canvas" id="traseraCanvas_{{ $i }}"></canvas>
                                        <div class="guide-frame" id="traseraGuideFrame_{{ $i }}">
                                            <div class="guide-message" id="traseraGuideMessage_{{ $i }}">Coloca el DNI dentro del marco</div>
                                        </div>
                                    </div>
                                    <div class="camera-controls">
                                        <button type="button" class="camera-btn camera-btn-capture" id="traseraCaptureBtn_{{ $i }}" disabled>📸 Capturar</button>
                                        <button type="button" class="camera-btn camera-btn-flip" id="traseraFlipBtn_{{ $i }}">🔄 Cambiar cámara</button>
                                        <button type="button" class="camera-btn camera-btn-retry" id="traseraRetryBtn_{{ $i }}" style="display: none;">🔄 Reintentar</button>
                                    </div>
                                </div>
                                <div id="traseraCameraPlaceholder_{{ $i }}" style="text-align: center;">
                                    <div class="upload-icon">📷</div>
                                    <div class="upload-text">Elige una opción</div>
                                    <div class="upload-hint">Puedes hacer una foto o subir una imagen</div>
                                    <div style="display: flex; gap: 12px; justify-content: center; margin-top: var(--spacing-md); flex-wrap: wrap;">
                                        <button type="button" class="camera-btn camera-btn-capture" onclick="initCamera({{ $i }}, 'trasera')">📸 Activar cámara</button>
                                        <button type="button" class="camera-btn camera-btn-capture" style="background: linear-gradient(135deg, #0D7377 0%, #0A5D61 100%);" onclick="document.getElementById('traseraFile_{{ $i }}').click()">📤 Subir archivo</button>
                                    </div>
                                </div>
                                <input type="file" id="traseraFile_{{ $i }}" name="trasera[{{ $i }}]" accept="image/jpeg,image/jpg,image/png" style="display: none;">
    ...
    <script>
        ...
        function removeImage(index, type) {
            ...
            // Ocultar wrapper de cámara si está visible
            if (wrapper) {
            ...
            // Eliminar controles de imagen (botones Eliminar/Reemplazar)
            const existingControls = area.querySelector('.image-controls');
            if (existingControls) {
                existingControls.remove();
            }
            ...
            if (placeholder) {
                ...
                // Restaurar placeholder con ambas opciones
                placeholder.innerHTML = `
                    <div class="upload-icon">📷</div>
                    <div class="upload-text">Elige una opción</div>
                    <div class="upload-hint">Puedes hacer una foto o subir una imagen</div>
                    <div style="display: flex; gap: 12px; justify-content: center; margin-top: var(--spacing-md); flex-wrap: wrap;">
                        <button type="button" class="camera-btn camera-btn-capture" onclick="initCamera(${index}, '${type}')">📸 Activar cámara</button>
                        <button type="button" class="camera-btn camera-btn-capture" style="background: linear-gradient(135deg, #0D7377 0%, #0A5D61 100%);" onclick="document.getElementById('${type}File_${index}').click()">📤 Subir archivo</button>
                    </div>
                `;
            }
            ...
            if (!processingState[index].frontal.processed && !processingState[index].trasera.processed) {
                ...
                if (additionalSection) {
                    ...
                    additionalSection.style.visibility = 'hidden';
                }
            }
            
            // Limpiar clase active del área
            if (area) {
        ...
        function replaceImage(index, type, event) {
            // Prevenir scroll si hay evento
            if (event) {
            ...
            // Reiniciar cámara
            initCamera(index, type);
            
            // Restaurar posición del scroll después de un breve delay
            // para evitar que el navegador haga scroll automático
            setTimeout(() => {
                window.scrollTo({
                    top: scrollY,
                    behavior: 'instant'
            ...
            if (addressFieldsContainer) {
                ...
                if (faltantes.direccion || faltantes.localidad || faltantes.codigo_postal || faltantes.provincia) {
                    ...
                    if (faltantes.codigo_postal) {
                        // Obtener código postal del campo hidden si existe
                        const codigoHidden = document.getElementById(`hidden_codigo_postal_${index}`);
                        let codigoValue = codigoHidden ? codigoHidden.value : '';
                        
                        // Si no hay valor en el hidden, verificar si ya existe un campo manual con valor
                        if (!codigoValue) {
                            const codigoManualExistente = document.getElementById(`codigo_postal_manual_${index}`);
                            if (codigoManualExistente && codigoManualExistente.value) {
                                codigoValue = codigoManualExistente.value;
                            }
                        }
                        ...
                        // Si no falta el código postal pero existe un campo manual, asegurarse de que tenga el valor del hidden
                        const codigoManualExistente = document.getElementById(`codigo_postal_manual_${index}`);
                        const codigoHidden = document.getElementById(`hidden_codigo_postal_${index}`);
                        if (codigoManualExistente && codigoHidden && codigoHidden.value && !codigoManualExistente.value) {
                            codigoManualExistente.value = codigoHidden.value;
                        }
                    }
        ...
        function removeRequiredFromFileInputs() {
            for (let i = 0; i < numeroAdultos; i++) {
                ...
                if (frontalFile) {
                    ...
                    // También eliminar del HTML directamente
                    if (frontalFile.hasAttribute('required')) {
                        frontalFile.removeAttribute('required');
                    }
                }
                if (traseraFile) {
                    ...
                    if (traseraFile.hasAttribute('required')) {
                        traseraFile.removeAttribute('required');
                    }
                }
            }
        ...
        // para evitar validación automática del navegador
        ...
        if (!submitBtn) {
            ...
            submitBtn.addEventListener('click', async function(e) {
                ...
                // Asegurar que los inputs de archivo no tienen required (por si acaso)
                removeRequiredFromFileInputs();
                
                // Verificar que se eliminaron correctamente y forzar eliminación
                for (let i = 0; i < numeroAdultos; i++) {
                    const frontalFile = document.getElementById(`frontalFile_${i}`);
                    const traseraFile = document.getElementById(`traseraFile_${i}`);
                    
                    if (frontalFile) {
                        // Múltiples intentos de eliminar required
                        frontalFile.removeAttribute('required');
                        frontalFile.required = false;
                        frontalFile.setAttribute('required', 'false');
                        frontalFile.removeAttribute('required');
                        // Verificar
                        if (frontalFile.hasAttribute('required') || frontalFile.required) {
                            console.warn(`⚠️ Input frontalFile_${i} todavía tiene required después de intentos, forzando eliminación...`);
                            frontalFile.removeAttribute('required');
                            frontalFile.required = false;
                        }
                    }
                    ...
                    if (traseraFile) {
                        traseraFile.removeAttribute('required');
                        traseraFile.required = false;
                        traseraFile.setAttribute('required', 'false');
                        traseraFile.removeAttribute('required');
                        if (traseraFile.hasAttribute('required') || traseraFile.required) {
                            console.warn(`⚠️ Input traseraFile_${i} todavía tiene required después de intentos, forzando eliminación...`);
                            traseraFile.removeAttribute('required');
                            traseraFile.required = false;
                        }
                    }
            ...
            try {
                console.log('🔵 Enviando petición a /dni-scanner/' + token + '/save-additional-data');
                console.log('🔵 FormData entries:', Array.from(formData.entries()));
                console.log('🔵 Número de adultos:', numeroAdultos);
                ...
                // Mostrar información de verificación en consola
                if (result.verificacion) {
                    console.log('✅ Verificación de datos guardados:', result.verificacion);
                    if (result.verificacion.datos) {
                        console.log('📋 Datos guardados:', result.verificacion.datos);
                    }
                    if (result.verificacion.fotos) {
                        console.log('📷 Fotos guardadas:', result.verificacion.fotos);
                    }
                }
                ...
                if (result.success) {
                    ...
                    // Usar redirect_url si está disponible, sino usar la ruta por defecto
                    const redirectUrl = result.redirect_url || `/dni-scanner/${token}`;
                    console.log('🔄 Redirigiendo a:', redirectUrl);
                    console.log('📊 Estado data_dni:', result.data_dni);
                    setTimeout(() => {
        ...
        // Función para prellenar campos manuales con valores de campos hidden
        function prefillManualFields() {
            for (let i = 0; i < numeroAdultos; i++) {
                // Prellenar código postal manual si existe y no tiene valor
                const codigoManual = document.getElementById(`codigo_postal_manual_${i}`);
                const codigoHidden = document.getElementById(`hidden_codigo_postal_${i}`);
                if (codigoManual && codigoHidden && codigoHidden.value && !codigoManual.value) {
                    codigoManual.value = codigoHidden.value;
                    console.log(`📝 Prellenado código postal manual ${i} con valor: ${codigoHidden.value}`);
                }
                
                // Prellenar otros campos manuales si es necesario
                const localidadManual = document.getElementById(`localidad_manual_${i}`);
                const localidadHidden = document.getElementById(`hidden_localidad_${i}`);
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function() {
                initializePage();
                // Prellenar campos manuales después de inicializar
                setTimeout(prefillManualFields, 500);
            });
        } else {
            // DOM ya está listo
            initializePage();
        ...
        const cameraStreams = {}; // Almacenar streams activos por índice y tipo
        ...
        // Inicializar cámara para un índice y tipo específico
        async function initCamera(index, type) {
            const videoId = `${type}Video_${index}`;
            const video = document.getElementById(videoId);
            const wrapper = document.getElementById(`${type}CameraWrapper_${index}`);
            const placeholder = document.getElementById(`${type}CameraPlaceholder_${index}`);
            const captureBtn = document.getElementById(`${type}CaptureBtn_${index}`);
            const flipBtn = document.getElementById(`${type}FlipBtn_${index}`);
            const retryBtn = document.getElementById(`${type}RetryBtn_${index}`);
            const guideFrame = document.getElementById(`${type}GuideFrame_${index}`);
            const guideMessage = document.getElementById(`${type}GuideMessage_${index}`);
            
            if (!video || !wrapper) {
                console.error('Elementos de cámara no encontrados', { index, type });
                return;
            }
            ...
            try {
                ...
                cameraStreams[streamKey] = stream;
                
                video.srcObject = stream;
                await video.play();
                
                // Habilitar botón de captura después de un momento
                setTimeout(() => {
                    captureBtn.disabled = false;
                    guideMessage.textContent = 'Coloca el DNI dentro del marco';
                }, 1000);
                
                // Iniciar detección básica
                startBasicDetection(video, guideFrame, guideMessage, captureBtn);
                
                // Configurar eventos de los botones
                captureBtn.onclick = () => capturePhoto(index, type);
                flipBtn.onclick = () => flipCamera(index, type);
                if (retryBtn) {
                    retryBtn.onclick = () => initCamera(index, type);
                }
        ...
        // Detección básica del documento (simplificada)
        function startBasicDetection(video, guideFrame, guideMessage, captureBtn) {
            ...
            const detect = () => {
                ...
                try {
                    // Simulación simple de detección - siempre habilitar captura
                    // En producción, aquí iría la lógica de detección real
                    if (!captureBtn.disabled) {
                        guideFrame.classList.add('detected');
                        guideMessage.textContent = '¡Listo para capturar!';
                        guideMessage.style.background = 'rgba(13, 115, 119, 0.9)';
                    }
                } catch (error) {
        ...
        // Capturar foto
        async function capturePhoto(index, type) {
            const videoId = `${type}Video_${index}`;
            const canvasId = `${type}Canvas_${index}`;
            const video = document.getElementById(videoId);
            ...
            if (!video || !canvas) {
                console.error('Elementos de captura no encontrados', { index, type });
                return;
            }
            ...
            try {
                ...
                const fileNameValue = `dni_${type}_${index}_${Date.now()}.jpg`;
                const file = dataURLtoFile(dataURL, fileNameValue);
                ...
                // Mostrar placeholder de nuevo (oculto) para que se pueda reactivar si es necesario
                placeholder.style.display = 'block';
                const activateBtn = placeholder.querySelector('button');
                if (activateBtn) {
                    activateBtn.textContent = '📷 Tomar otra foto';
                    activateBtn.onclick = () => {
                        // Limpiar preview y reactivar cámara
                        preview.classList.remove('active');
                        previewImage.src = '';
                        fileName.textContent = '';
                        const fileInput = document.getElementById(`${type}File_${index}`);
                ...
                // Simular el input file para que funcione con la lógica existente
                const fileInput = document.getElementById(`${type}File_${index}`);
                if (fileInput) {
                    ...
                    // Disparar evento change para que se procese automáticamente
        ...
        async function flipCamera(index, type) {
            const videoId = `${type}Video_${index}`;
            const video = document.getElementById(videoId);
            const flipBtn = document.getElementById(`${type}FlipBtn_${index}`);
            ...
            try {
                ...
                const currentFacingMode = video.srcObject?.getVideoTracks()[0]?.getSettings()?.facingMode || 'environment';
                ...
                flipBtn.disabled = false;
                alert('Error al cambiar la cámara: ' + error.message);
            }
        }
