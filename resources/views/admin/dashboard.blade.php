...
<div class="container-fluid">
    ...
    {{-- [DEMO 2026-04-17] Banner y hero hotelero — solo visibles si DEMO_MODE=true --}}
    @if(config('demo.demo_mode'))
    <style>
        .demo-hero {
            background: linear-gradient(135deg, #1b3a57 0%, #2b5a85 50%, #0f2740 100%);
            color: #fff;
            border-radius: 12px;
            padding: 2rem 2.5rem;
        ...
        .demo-hero::before {
            ...
            position: absolute;
            inset: 0;
            background-image: radial-gradient(circle at 85% 30%, rgba(255,255,255,0.08), transparent 40%),
                              radial-gradient(circle at 15% 70%, rgba(255,255,255,0.06), transparent 35%);
            pointer-events: none;
        }
        .demo-hero h1 { font-weight: 700; letter-spacing: 0.5px; margin-bottom: 0.25rem; }
        .demo-hero .categoria { font-size: 0.95rem; opacity: 0.85; }
        .demo-hero .meta { font-size: 0.82rem; opacity: 0.75; margin-top: 0.5rem; }
        .demo-banner-warn {
            background: #ffc107;
            color: #1a1a1a;
        ...
        .kpi-hotel .label { color: #6b7280; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.8px; }
        .kpi-hotel .value { color: #1b3a57; font-size: 1.75rem; font-weight: 700; line-height: 1.1; margin-top: 0.25rem; }
        .kpi-hotel .sub { color: #9ca3af; font-size: 0.75rem; margin-top: 0.2rem; }
        .kpi-hotel .icon { float: right; opacity: 0.25; font-size: 1.8rem; color: #2b5a85; margin-top: -0.2rem; }
    </style>
    <div class="demo-banner-warn">
        <i class="fa-solid fa-triangle-exclamation"></i>
        {{ __('hotel.demo_banner') }}
    </div>
    <div class="demo-hero">
        <div class="row align-items-center">
            <div class="col-md-8">
                <h1>{{ config('demo.hotel.nombre', 'Hotel Demo') }}</h1>
                <div class="categoria">
                    <i class="fa-solid fa-star"></i> {{ config('demo.hotel.categoria', '4 estrellas') }}
                    <span class="mx-2">·</span>
                    <i class="fa-solid fa-location-dot"></i> {{ config('demo.hotel.ubicacion', 'Costa del Sol') }}
                </div>
                <div class="meta">
                    Panel de gestión hotelera
                    <span class="mx-2">·</span>
                    <i class="fa-regular fa-calendar"></i> {{ now()->translatedFormat('l, d \d\e F \d\e Y') }}
                </div>
            </div>
            <div class="col-md-4 text-end">
                <div style="font-size: 3rem; opacity: 0.35;">
    ...
    {{-- KPIs hoteleros demo (ocupación, ADR, etc.) --}}
    @php
        // Cálculos ligeros — solo tocan datos ya cargados por el controller
        // si existen, en otro caso fallback inofensivo.
        $kpi_totalHabitaciones = \App\Models\Apartamento::count() ?: 20;
        $hoy = \Carbon\Carbon::today();
        $kpi_ocupadasHoy = \App\Models\Reserva::whereDate('fecha_entrada', '<=', $hoy)
            ->whereDate('fecha_salida', '>', $hoy)
            ->where('estado_id', '!=', 4)
            ->count();
        $kpi_ocupacionPct = $kpi_totalHabitaciones > 0
            ? round(($kpi_ocupadasHoy / $kpi_totalHabitaciones) * 100)
            : 0;
        $kpi_checkinsHoy = \App\Models\Reserva::whereDate('fecha_entrada', $hoy)
            ->where('estado_id', '!=', 4)->count();
        $kpi_checkoutsHoy = \App\Models\Reserva::whereDate('fecha_salida', $hoy)
            ->where('estado_id', '!=', 4)->count();
        $kpi_ingresosMes = \App\Models\Invoices::whereMonth('fecha', now()->month)
            ->whereYear('fecha', now()->year)
            ->sum('total');
        $kpi_noches_mes = \App\Models\Reserva::whereMonth('fecha_salida', now()->month)
            ->whereYear('fecha_salida', now()->year)
            ->where('estado_id', '!=', 4)
            ->get()
            ->sum(function($r) {
                return \Carbon\Carbon::parse($r->fecha_entrada)->diffInDays($r->fecha_salida);
            });
        $kpi_adr = $kpi_noches_mes > 0 ? round($kpi_ingresosMes / $kpi_noches_mes, 2) : 0;
        $kpi_facturasPendientes = \App\Models\Invoices::where('invoice_status_id', 1)->count();
    @endphp
    <div class="row g-3 mb-4">
        <div class="col-md-3 col-sm-6">
            <div class="kpi-hotel">
                <i class="fa-solid fa-bed icon"></i>
                <div class="label">{{ __('hotel.kpi_ocupacion') }}</div>
                <div class="value">{{ $kpi_ocupacionPct }}%</div>
                <div class="sub">{{ $kpi_ocupadasHoy }} / {{ $kpi_totalHabitaciones }} habitaciones</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="kpi-hotel">
                <i class="fa-solid fa-euro-sign icon"></i>
                <div class="label">{{ __('hotel.kpi_adr') }}</div>
                <div class="value">{{ number_format($kpi_adr, 0, ',', '.') }} €</div>
                <div class="sub">media del mes</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="kpi-hotel">
                <i class="fa-solid fa-chart-line icon"></i>
                <div class="label">{{ __('hotel.kpi_ingresos_mes') }}</div>
                <div class="value">{{ number_format($kpi_ingresosMes, 0, ',', '.') }} €</div>
                <div class="sub">facturado este mes</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="kpi-hotel">
                <i class="fa-solid fa-right-to-bracket icon"></i>
                <div class="label">{{ __('hotel.kpi_checkins_hoy') }} / check-outs</div>
                <div class="value">{{ $kpi_checkinsHoy }} / {{ $kpi_checkoutsHoy }}</div>
                <div class="sub">{{ $kpi_facturasPendientes }} facturas pendientes</div>
            </div>
        </div>
    </div>
    @endif
