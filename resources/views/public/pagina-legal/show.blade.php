                    ...
                    @foreach($paginasSidebar as $paginaSidebar)
                        <li>
                            <a href="{{ route('web.pagina-legal.show', $paginaSidebar->slug) }}" 
                               class="{{ $pagina->slug === $paginaSidebar->slug ? 'active' : '' }}">
        ...
        <main class="politica-main">
            <div class="politica-header">
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                    <div>
                        <h1 class="politica-title">{{ translate_dynamic($pagina->titulo) }}</h1>
                        @if($pagina->fecha_actualizacion)
                            <p class="politica-date">
                                {{ __('legal.updated_on') }} {{ $pagina->fecha_actualizacion->format('d') }} {{ __('legal.month_' . strtolower($pagina->fecha_actualizacion->format('F'))) }} {{ $pagina->fecha_actualizacion->format('Y') }}
                            </p>
                        @endif
                    </div>
            ...
            <div class="politica-content">
                {!! translate_dynamic($pagina->contenido) !!}
            </div>
        </main>
    </div>
</div>
